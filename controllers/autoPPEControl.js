...
const submitPPE = async (req, res) => {
    try {
        ...
        if (!req.files || !req.files['ppeimg'] || !req.files['ppeimg'][0]) {
            return res.status(400).json({
                error: "Company image is missing"
            });
        }

        const ppeImg = req.files['ppeimg'][0].filename;
        ...
        // Create user record
        const newUser = await PPEUser.create({
            first,
            last,
            company,
            email,
        ...
        const response = {
            message: 'PPE Sale submitted successfully',
