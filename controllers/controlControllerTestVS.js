...
// For local dev: set APP_URL=http://localhost:5173
...
const manageLinks = createdJobs.map(job =>
        job.jobDates.map(jobDateObj => {
          const dateString = new Date(jobDateObj.date).toLocaleDateString('en-US');
  return `<li><a href="http://localhost:5173/manage-job/${job._id}">${dateString} – Edit this job</a></li>`;
}).join('')).join('');
const workOrderLinks = createdJobs
  .map(job =>
    job.jobDates.map(jobDateObj => {
      const human = new Date(jobDateObj.date).toLocaleDateString('en-US');
      const iso = new Date(jobDateObj.date).toISOString().split('T')[0]; // YYYY-MM-DD
      const link = `${APP_URL}/work-order/${job._id}?date=${iso}`;
      return `<li>${human} – <a href="${link}">Open Work Order</a></li>`;
    }).join('')
  ).join('');
            ...
            <html>
              <body style="margin: 0; padding: 20px; font-family: Arial, sans-serif; background-color: #e7e7e7; color: #000;">
                <div style="max-width: 600px; margin: auto; background: #fff; padding: 20px; border-radius: 8px;">
                  ...
                  <p>${message}</p>
<h3><strong>IMPORTANT: </strong>Please fill out work order once job is completed and review carefully before submitting the work order:</h3>
<ul>
  ${workOrderLinks}
</ul>
